...
def read_users() -> dict:
    """讀取 JSON 並回傳 dict；若檔案不存在或格式錯誤則回傳空清單"""
    try:
        with open(JSON_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
...
def validate_register(form_data: dict) -> dict:
    """
    驗證註冊表單資料，檢查格式、長度與重複性。
    驗證通過則寫入 JSON，失敗則回傳穩定錯誤結構。
    """
...
def register_route() -> str:
    ...
    if request.method == 'POST':
        ...
        result = validate_register(form_data)
        return redirect(url_for('login_route') if result["success"] else url_for('error_route', message=result["error"]))
...
def login_route() -> str:
    ...
    if request.method == 'POST':
        ...
        result = verify_login(email, password, read_users().get("users", []))
        if result["success"]:
...
def welcome_route(username: str) -> str:
    ...
    # 使用生成器表達式快速比對
    target = next((u for u in read_users().get("users", []) if u.get("username") == username), None)
    if target is None:
...
def users_list_route() -> str:
    """會員清單頁：讀取 JSON 並渲染表格"""
    return render_template('users.html', title='會員清單', users=read_users().get("users", []))
